...
import './Hunt.css'
import HuntData from './HuntData.jsx';
...
const { addToCart } = useCart();
  ...
  return (
    <div>
      <section className="HuntSection">
        <div className="HuntHead">
          <h3>Best selling For Hunt:</h3>
        </div>
        <div className="HuntScrollContainer">
          <button className="scroll-btn left" onClick={() => scroll(-300)}>
            ❮
          </button>
          <div className="Huntcollection" ref={scrollRef}>
            {infiniteHuntdata.map((item, index) => (
              <div className="HuntcollectCard" key={index}>
                <div className="HuntImgContain">
                  <img src={item.image} alt="" />
                </div>
                <div className="HuntContent">
                  <p>₹ {item.price}</p>
                  <h4>{item.title}</h4>
                </div>
